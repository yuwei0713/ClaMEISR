...
<body>
    ...
    <div id="actualpage" class="actualpage" style="display:none;">
        <script>
            $(function() {
                $(".flip").click(function() {
        ...
        <div class="choosetopic container">
            ...
            <div class="panel">
                @for ($i = 0; $i < count($HistoryData); $i++)
                    <label class="chooseframeworklabel" for="span{{ $HistoryData[$i]['BigTopicNumber'] }}"
                        id="label{{ $HistoryData[$i]['BigTopicNumber'] }}">
                        <div class="chooseframework" onclick="changeshow({{ $HistoryData[$i]['BigTopicNumber'] }})">
                            <span
                                id="span{{ $HistoryData[$i]['BigTopicNumber'] }}">{{ $HistoryData[$i]['BigTopicName'] }}</span>
                        </div>
                    </label>
                @endfor
            </div>
                            ...
                            <div>
                                <div>
                                    <div>
                                        ...
                                        <div class="rTable option-table"
                                            id="checktable{{ $HistoryData[$i]['BigTopicNumber'] }}">
                                            @php
                                                $child_flag = 0;
                                            @endphp
                                            <input type="hidden" id="Topic{{ $HistoryData[$i]['BigTopicNumber'] }}"
                                                name="Topic{{ $HistoryData[$i]['BigTopicNumber'] }}"
                                                value="{{ count($HistoryData[$i]['Detail']) }}">
                                            ...
                                            @foreach ($HistoryData[$i]['Detail'] as $detail)
                                                @if ($ChildBasicData->Age >= $detail['SuitableAge'])
                                                    <div class="rTableRow topic-rows"
                                                        id="q{{ $HistoryData[$i]['BigTopicNumber'] }}-{{ $detail['SmTopicNumber'] }}">
                                                    @else
                                                        @if ($child_flag == 0)
                                                            @php
                                                                $child_flag = 1;
                                                            @endphp
                                                            <div class="rTableRow topic-rows not-yet over_age"
                                                                id="q{{ $HistoryData[$i]['BigTopicNumber'] }}-{{ $detail['SmTopicNumber'] }}">
                                                            @elseif($child_flag == 1)
                                                                <div class="rTableRow topic-rows over_age"
                                                                    id="q{{ $HistoryData[$i]['BigTopicNumber'] }}-{{ $detail['SmTopicNumber'] }}">
                                                        @endif
                                                @endif
                                                ...
                                                @for ($k = 0; $k < count($detail['OptionValue']); $k++)
                                                    <label class="rwd-option-framework rTableCell">
                                                        <div class="option-css"
                                                            data-th="{{ $HistoryData[$i]['OptionContent'][$k] }}">
                                                            <span class="option-size">
                                                                <span class="option-size-adjust">
                                                                    <div class="option-size-inner">
                                                                        @if ($detail['FillTopic'] == $k + 1)
                                                                            <input type="radio"
                                                                                name="q{{ $HistoryData[$i]['BigTopicNumber'] }}-{{ $detail['SmTopicNumber'] }}"
                                                                                value="{{ $k + 1 }}-{{ $detail['OptionValue'][$k] }}"
                                                                                class="option-circle" checked>
                                                                        @else
                                                                            <input type="radio"
                                                                                name="q{{ $HistoryData[$i]['BigTopicNumber'] }}-{{ $detail['SmTopicNumber'] }}"
                                                                                value="{{ $k + 1 }}-{{ $detail['OptionValue'][$k] }}"
                                                ...
                                                @for ($j = 0; $j < count($detail['AdditionContent']); $j++)
                                                    <div class="rTableCell represent-css option-css">
    ...
    <div class="send-framwork">
        <div class="pre-page">
            <button type="button" class="pre-button"
                onclick="location.href='{{ url('/front') }}'"><span>回首頁</span></button>
        </div>
        ...
        @else
            <div class="next-page" id="finalpagebutton">
                <button type="button" class="next-button"
                    onclick="changeshow({{ $HistoryData[$i]['BigTopicNumber'] + 1 }})"><span>下一頁</span></button>
            </div>
        @endif
    </div>
    <!--進度條-->
    <div class="css-13z4rfv">
        <div class="css-1kvwmfc progress-moved">
            <progress class="progress-framework" id="progress{{ $HistoryData[$i]['BigTopicNumber'] }}"
                max="" value=""></progress>
